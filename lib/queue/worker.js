...
var EventEmitter = require('events').EventEmitter
    ...
    , debug = require('debug')('digit:kue:worker');
...
Worker.prototype.start = function (fn) {
    ...
    self.getJob(function (err, job) {
        if (err) { debug('error on getJob call from start'); debug(JSON.stringify(err)); };
...
Worker.prototype.getJob = function (fn) {
    ...
    client.blpop('q:' + self.type + ':jobs', 0, function (err) {
        debug('popped value from ' + self.type + ' queue');
        if (err) debug('got an error');
        if (err) debug('error is ' + JSON.stringify(err));
        if (err) return fn(err);		// SAE: Added to avoid crashing redis on zpop
        if (!self.running) {
            debug('lpushing in !running logic branch of getJob');
...
Worker.prototype.shutdown = function (fn, timeout) {
    ...
    var _fn = function (job) {
        ...
        debug('lpushing in shutdown function');
