...
var EventEmitter = require('events').EventEmitter
    ...
    , reds = require('reds')
    , _ = require('lodash')
    , debug = require('debug')('digit:kue:job')
    , noop = function () {
    };
...
//            console.log( "setState(%d) End ", this.id, state, this._state/*, replies*/ );
        ...
        this.set('state', state, function(){
            ...
            if ('inactive' == state) { debug('lpushing in set state call from prototype.state function'); };
...
Job.prototype.failed = function (clbk) {
    return this.set('failed_at', Date.now(), function(){ this.state('failed', clbk); }.bind(this));
};
